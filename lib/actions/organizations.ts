...
export async function createOrganization(input: CreateOrgInput) {
  ...
  const defaultPkg = await prisma.package.findFirst({ orderBy: { price: 'asc' } })
...
export async function inviteResearcher(formData: FormData) {
  ...
  const pendingInvite = await prisma.orgInvitation.findFirst({
    where: { orgId: org.id, email, usedAt: null, expiresAt: { gt: new Date() } },
  })
  if (pendingInvite) throw new Error('Bu e-postaya zaten aktif bir davet gönderilmiş.')
...
export async function deactivateOrgMember(memberId: string) {
  ...
  if (member.role === 'org_admin') throw new Error('Org admin kendi hesabını devre dışı bırakamaz.')

  await prisma.orgMember.update({ where: { id: memberId }, data: { isActive: false } })
  revalidatePath('/org-admin/members')
}
...
export async function reactivateOrgMember(memberId: string) {
  ...
  await prisma.orgMember.update({ where: { id: memberId }, data: { isActive: true } })
  revalidatePath('/org-admin/members')
}
