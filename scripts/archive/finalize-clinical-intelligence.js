...
// 1. AI Tanı Sayfasını (JSON Parser) Güncelleme
const diagnosisPath = 'app/(dashboard)/ai-diagnosis/page.tsx';
const diagnosisCode = `"use client";
...
try {
  fs.writeFileSync(diagnosisPath, diagnosisCode);
  console.log("✅ AI Tanı: Dashboard Görünümü ve JSON Parser Kuruldu!");

  // 2. OSCE Sınav Sonucu Kayıt Mekanizması (Prompt Güncelleme)
  const routerPath = 'lib/ai/router.ts';
  let routerCode = fs.readFileSync(routerPath, 'utf8');
  routerCode = routerCode.replace('Geri Bildirim', 'Geri Bildirim ve "ZAYIF ALAN ANALİZİ"');
  fs.writeFileSync(routerPath, routerCode);
  console.log("✅ OSCE: Profil Güncelleme Talimatı Eklendi!");
