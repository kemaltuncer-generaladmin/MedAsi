...
const navItems = [
  { href: '/org-admin',         icon: LayoutDashboard, label: 'Genel Bakış',   exact: true },
    ...
    >
      ...
      <div className="h-16 flex items-center gap-2.5 px-5 border-b" style={{ borderColor: 'var(--color-border)' }}>
        ...
        <div className="min-w-0">
          <p className="text-sm font-bold truncate" style={{ color: 'var(--color-text-primary)' }}>{orgName}</p>
          <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>Araştırma Paneli</p>
        </div>
      </div>
      ...
      <nav className="flex-1 py-3 px-2 space-y-0.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon size={15} style={{ color: isActive ? 'var(--color-primary)' : 'currentColor' }} />
              {item.label}
            </Link>
          )
        })}
