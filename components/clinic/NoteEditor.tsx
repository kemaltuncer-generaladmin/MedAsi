...
export function NoteEditor({ patientId }: NoteEditorProps) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(
        (j?.data || []).filter((n: NoteItem) => n.patientId === patientId),
      );
    } catch {
  ...
  const remove = async (id: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/clinic/notes?id=${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        setError(j?.error || "Silme başarısız");
  ...
  const allTags = useMemo(
    () => Array.from(new Set(items.flatMap((n) => n.tags))),
    [items],
  );
    ...
    <Card variant="bordered" className="rounded-xl p-6 sm:p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" className="h-11" onClick={() => void load()}>
            <Plus size={16} />
            Yenile
          </Button>
          <Button
