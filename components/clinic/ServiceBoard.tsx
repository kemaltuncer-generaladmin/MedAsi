...
export function ServiceBoard({ patientId }: ServiceBoardProps) {
  ...
  const [status, setStatus] = useState<'all' | 'requested' | 'in_progress' | 'completed'>('all')
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems((j?.data || []).filter((x: ServiceItem) => x.patientId === patientId))
    } catch {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((x) => {
      const s = !q || x.type.toLowerCase().includes(q) || (x.note || '').toLowerCase().includes(q)
  ...
  const submit = async () => {
    const parsed = serviceSchema.safeParse(editing ? { ...form, id: editing.id } : form)
    if (!parsed.success) {
    ...
    <>
      <Card variant="bordered" className="rounded-xl p-6 sm:p-8">
        <div className="mb-6 flex items-center gap-2">
          ...
          <h3 className="text-xl font-bold text-[var(--color-text-primary)]">Servis İşlemleri</h3>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="relative">
            ...
            <Input className="h-11 pl-10" placeholder="Ara..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="relative">
            ...
            <select
              ...
              value={status}
              onChange={(e) => setStatus(e.target.value as 'all' | 'requested' | 'in_progress' | 'completed')}
            >
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Input className="h-11" placeholder="Hizmet tipi" value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))} />
          <select
            className="h-11 rounded-md border border-[var(--color-border)] bg-[var(--color-background)] px-3 text-[var(--color-text-primary)]"
            value={form.status}
            onChange={(e) => setForm((p) => ({ ...p, status: e.target.value as ServiceInput['status'] }))}
          >
            <option value="requested">İstendi</option>
            <option value="in_progress">Devam</option>
            <option value="completed">Tamamlandı</option>
          </select>
          <Input className="h-11" placeholder="Not" value={form.note || ''} onChange={(e) => setForm((p) => ({ ...p, note: e.target.value }))} />
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button className="h-11 rounded-xl" onClick={() => void submit()} disabled={loading}>
        ...
        <div className="mt-6">
          {!filtered.length ? (
            ...
            <div className="space-y-3">
              {filtered.map((x) => (
                <div key={x.id} className="rounded-xl border border-[var(--color-border)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-medium text-[var(--color-text-primary)]">{x.type}</p>
                      <p className="text-sm text-[var(--color-text-secondary)]">{x.note || '-'}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="ghost" className="h-9" onClick={() => beginEdit(x)}><Edit size={14} /></Button>
                      <Button variant="ghost" className="h-9" onClick={() => setConfirmId(x.id)}><Trash2 size={14} /></Button>
                    </div>
                  </div>
                </div>
              ))}
