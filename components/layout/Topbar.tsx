...
export function Topbar({ user }: TopbarProps) {
  ...
  return (
    <header className="h-topbar border-b border-[var(--color-border)] bg-[var(--color-surface)] flex items-center justify-between px-6 fixed top-0 left-sidebar right-0 z-40">
      <div className="flex items-center gap-2">
        <h1 className="text-base font-semibold text-[var(--color-text-primary)]">{greeting},</h1>
        <span className="text-base font-semibold text-[var(--color-primary)]">{user?.name || 'Kullanıcı'}</span>
      </div>
