...
import {
  ShieldCheck, ArrowLeft, Bell, Plus, X, Edit2, Trash2,
  ChevronDown, ChevronUp, Eye, Info, AlertTriangle,
  CheckCircle2, AlertCircle, Calendar, Users, Clock,
  type LucideIcon
} from 'lucide-react'
import { Badge } from '@/components/ui/Badge'
import { Button } from '@/components/ui/Button'

const LS_KEY = 'medasi_admin_announcements_v1'
...
const TYPE_CONFIG: Record<Announcement['type'], {
  label: string
  color: string
  borderColor: string
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-[var(--color-text-primary)]">{announcement.title}</p>
        <p className="text-xs text-[var(--color-text-secondary)] mt-0.5 leading-relaxed">{announcement.message}</p>
      </div>
    </div>
  )
}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex items-center gap-2 min-w-0">
            <Icon size={15} style={{ color: config.color }} className="shrink-0" />
            <p className="text-sm font-bold text-[var(--color-text-primary)] truncate">{announcement.title}</p>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              type="button"
              onClick={() => {
                if (window.confirm(`"${announcement.title}" duyurusunu silmek istediğinizden emin misiniz?`)) {
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          ...
          <Badge variant="secondary">{TARGET_CONFIG[announcement.target].label}</Badge>
          ...
          <span className="flex items-center gap-1 text-[var(--color-text-secondary)]">
            <Calendar size={10} />
            {announcement.expiresAt ? formatDate(announcement.expiresAt) : 'Süresiz'}
          </span>
        </div>
      </div>
    </div>
...
export default function AnnouncementsPage() {
  ...
  const previewAnnouncement: Announcement = {
    ...
    message: form.message || 'Duyuru mesajı buraya gelecek. Kullanıcılar bu metni görecek.',
    type: form.type,
    target: form.target,
    active: true,
    createdAt: new Date().toISOString(),
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Duyuru Yönetimi</h1>
          </div>
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-1.5">
                    Başlık <span className="text-[var(--color-destructive)]">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.title}
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-1.5">
                    Mesaj <span className="text-[var(--color-destructive)]">*</span>
                  </label>
                  <textarea
                    value={form.message}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-md border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent"
                    >
                      {(Object.keys(TARGET_CONFIG) as Announcement['target'][]).map((t) => (
                        <option key={t} value={t}>{TARGET_CONFIG[t].label}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider mb-1.5">
                    Bitiş Tarihi <span className="text-[var(--color-text-secondary)] font-normal">(isteğe bağlı)</span>
                  </label>
                  <input
                    type="date"
                  ...
                  />
                  {!form.expiresAt && (
                    <p className="text-xs text-[var(--color-text-secondary)] mt-1">Belirtilmezse süresiz gösterilir</p>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="rounded-lg border border-[var(--color-border)] p-4 space-y-3 bg-[var(--color-background)]">
                  <p className="text-xs text-[var(--color-text-secondary)] mb-2">Dashboard'da şöyle görünecek:</p>
                  <AnnouncementBanner announcement={previewAnnouncement} />

                  <div className="pt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-[var(--color-text-secondary)]">Tür:</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[var(--color-text-secondary)]">
                        {form.expiresAt ? `${formatDate(form.expiresAt)} tarihinde sona erer` : 'Süresiz'}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold text-[var(--color-text-primary)]">Aktif Duyurular</h2>
              {active.length > 0 && (
                <span
          ...
          {active.length === 0 ? (
            <div className="bg-[var(--color-surface-elevated)] rounded-xl p-10 text-center">
              <Bell size={32} className="text-[var(--color-text-secondary)] mx-auto mb-3 opacity-40" />
              <p className="text-sm font-medium text-[var(--color-text-secondary)]">Aktif duyuru yok</p>
              <p className="text-xs text-[var(--color-text-secondary)] mt-1 mb-4">
                Yeni bir duyuru oluşturun veya pasif duyurulardan birini aktifleştirin.
              </p>
              <Button variant="primary" size="sm" onClick={openCreate}>
                <Plus size={14} />
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold text-[var(--color-text-primary)]">Pasif Duyurular</h2>
              {inactive.length > 0 && (
            ...
            <div className="mt-3 space-y-3">
              {inactive.length === 0 ? (
                <div className="bg-[var(--color-surface-elevated)] rounded-xl p-8 text-center">
                  <p className="text-sm text-[var(--color-text-secondary)]">Pasif duyuru yok</p>
                </div>
              ) : (
                inactive.map((a) => (
                  <AnnouncementCard
