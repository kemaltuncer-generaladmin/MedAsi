...
type Tab = 'patients' | 'labs' | 'service' | 'prescriptions' | 'discharge' | 'notes'
...
export default function ClinicPage() {
  ...
  const loadPatients = async () => {
    ...
    if (!selectedPatientId && json?.data?.[0]?.id) setSelectedPatientId(json.data[0].id)
  }
  ...
  const deletePatient = async (id: string) => {
    ...
    if (!ok) return
    try {
      const res = await fetch(`/api/clinic/patients?id=${id}`, { method: 'DELETE' })
      if (!res.ok) {
    ...
    <div className="space-y-6">
      <Card variant="bordered" className="rounded-xl p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Klinik Yönetimi</h1>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant={tab === 'patients' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('patients')}><UserPlus size={16} />Hastalar</Button>
            <Button variant={tab === 'labs' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('labs')}><Beaker size={16} />Lab</Button>
            <Button variant={tab === 'service' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('service')}><Bed size={16} />Servis</Button>
            <Button variant={tab === 'prescriptions' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('prescriptions')}><Pill size={16} />Reçete</Button>
            <Button variant={tab === 'discharge' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('discharge')}><FileCheck size={16} />Taburcu</Button>
            <Button variant={tab === 'notes' ? 'primary' : 'ghost'} className="h-11" onClick={() => setTab('notes')}>Notlar</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[420px_1fr]">
          ...
          <Card variant="bordered" className="rounded-xl p-6">
            {!filteredPatients.length ? (
              <div className="rounded-xl border border-dashed border-[var(--color-border)] p-8 text-center text-sm text-[var(--color-text-secondary)]">Hasta kaydı bulunamadı.</div>
            ) : (
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                {filteredPatients.map((patient) => (
                  <div key={patient.id} onClick={() => setSelectedPatientId(patient.id)}>
                    <PatientCard
                      patient={patient}
      ...
      {tab === 'labs' ? (
        selectedPatientId ? (
          ...
          <Card variant="bordered" className="rounded-xl p-8 text-sm text-[var(--color-text-secondary)]">Önce hasta seç.</Card>
        )
      ) : null}
