...
import { TUS_TOPIC_GROUPS } from "@/constants/tus";
...
const STATUS_CYCLE: StudyStatus[] = [
  "Çalışmadım",
  "Çalışıyorum",
  "Tamamlandım",
  "Tekrar Gerekli",
...
const SUBJECT_GROUPS: SubjectGroup[] = TUS_TOPIC_GROUPS.map((group) => ({
  name: group.name,
...
export default function TusPlannerPage() {
  ...
  const allSubtopics = SUBJECT_GROUPS.flatMap((g) =>
    g.subtopics.map((s) => groupKey(g.name, s)),
  );
  const total = allSubtopics.length;
  const completed = allSubtopics.filter(
    (k) => statusMap[k] === "Tamamlandım",
  ).length;
  const inProgress = allSubtopics.filter(
    (k) => statusMap[k] === "Çalışıyorum",
  ).length;
  const needsRepeat = allSubtopics.filter(
    (k) => statusMap[k] === "Tekrar Gerekli",
  ).length;
  ...
  function groupProgress(group: SubjectGroup) {
    const done = group.subtopics.filter(
      (s) => statusMap[groupKey(group.name, s)] === "Tamamlandım",
    ).length;
    return {
      done,
      total: group.subtopics.length,
      pct: Math.round((done / group.subtopics.length) * 100),
    };
  }
    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              TUS sınav hazırlık takip sistemi
            </p>
          </div>
        </div>
      ...
      <Card variant="bordered" className="p-4">
        ...
        <p className="text-xs text-[var(--color-text-secondary)] mt-1">
          Konulara tıklayarak durumunu döngüsel olarak değiştir
        </p>
      </Card>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <span className="text-xs text-[var(--color-text-secondary)] ml-2 w-8 text-right">
                  %{prog.pct}
                </span>
              </button>
              {isOpen && (
      ...
      <Card variant="bordered" className="p-4">
        ...
        <p className="text-xs text-[var(--color-text-secondary)] mt-2 opacity-70">
          Konuya tıkladıkça durum sırayla değişir
        </p>
      </Card>
    </div>
  );
