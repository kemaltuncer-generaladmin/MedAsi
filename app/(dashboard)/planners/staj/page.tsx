...
export default function StajPlannerPage() {
  ...
  const overallPct =
    totalProcs > 0 ? Math.round((doneProcs / totalProcs) * 100) : 0;
  ...
  function addProcedure(rotationId: string) {
    ...
    setRotations((prev) =>
      prev.map((r) =>
        r.id === rotationId ? { ...r, procedures: [...r.procedures, proc] } : r,
      ),
    );
  ...
  function deleteProcedure(rotationId: string, procId: string) {
    setRotations((prev) =>
      prev.map((r) =>
        r.id === rotationId
          ? { ...r, procedures: r.procedures.filter((p) => p.id !== procId) }
          : r,
      ),
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              Klinik staj rotasyonları ve prosedür takibi
            </p>
          </div>
        </div>
        ...
        <Card variant="bordered" className="p-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium text-[var(--color-text-primary)]">
              Tüm Prosedürlerde İlerleme
            </p>
            <p className="text-sm text-[var(--color-text-secondary)]">
              {doneProcs}/{totalProcs} tamamlandı
            </p>
          </div>
            ...
            <div className="space-y-3 mb-4">
              <div>
                ...
                <p className="text-xs text-[var(--color-text-secondary)] mt-1 opacity-70">
                  Bilinen bölümler için varsayılan prosedürler otomatik eklenir
                </p>
              </div>
              ...
              >
                ...
                {isOpen && (
                  <div className="px-4 pb-4 border-t border-[var(--color-border)]">
                    <p className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide my-3">
                      Prosedürler
                    </p>
                    <div className="space-y-1.5 mb-3">
                      ...
                      {rot.procedures.map((proc) => (
                        <div
                          key={proc.id}
                          className="flex items-center gap-2 p-2 rounded-lg hover:bg-[var(--color-surface-elevated)] transition-colors"
                        >
                          <button
                            onClick={() => toggleProcedure(rot.id, proc.id)}
                          >
                            {proc.done ? (
                              <CheckCircle2
                                size={16}
                          ...
                          <span
                            className={`text-sm flex-1 ${proc.done ? "line-through text-[var(--color-text-secondary)]" : "text-[var(--color-text-primary)]"}`}
                          >
                            {proc.label}
                          </span>
                          <button
                            onClick={() => deleteProcedure(rot.id, proc.id)}
                          >
                            <X
                              size={11}
                              className="text-[var(--color-destructive)] opacity-30 hover:opacity-100"
                            />
                          </button>
                        </div>
                      ))}
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && addProcedure(rot.id)
                        }
                      ...
                      />
                      <Button size="sm" onClick={() => addProcedure(rot.id)}>
                        <Plus size={12} />
                      </Button>
                    </div>
                  </div>
