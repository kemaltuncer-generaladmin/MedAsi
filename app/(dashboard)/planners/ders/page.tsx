...
import { TUS_SUBJECTS_WITH_OTHER, TUS_SUBJECT_COLORS } from "@/constants/tus";
...
const SUBJECTS = TUS_SUBJECTS_WITH_OTHER.map((label) => ({
  label,
  color:
    label === "Diğer"
      ? "var(--color-text-secondary)"
      : TUS_SUBJECT_COLORS[label],
}));
...
export default function DersPlannerPage() {
  ...
  const [formSubject, setFormSubject] = useState<string>(SUBJECTS[0].label);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              Haftalık ders programı ve çalışma takibi
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {tasks.length > 0 && (
                  <span className="text-xs text-[var(--color-text-secondary)]">
                    {(dayDone / 60).toFixed(1)}/{(dayTotal / 60).toFixed(1)}s
                  </span>
                )}
              </div>
              {tasks.length === 0 ? (
                <p className="text-xs text-[var(--color-text-secondary)] text-center py-4 opacity-50">
                  Görev yok
                </p>
              ) : (
                <div className="space-y-2">
                  {tasks.map((task) => (
                    <div
                      ...
                      style={{
                        borderColor: `color-mix(in srgb, ${getSubjectColor(task.subject)} 40%, transparent)`,
                        background: `color-mix(in srgb, ${getSubjectColor(task.subject)} 8%, transparent)`,
                      }}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-xs font-medium truncate ${task.done ? "line-through text-[var(--color-text-secondary)]" : "text-[var(--color-text-primary)]"}`}
                          >
                            {task.topic}
                          </p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span
                              ...
                              style={{ color: getSubjectColor(task.subject) }}
                            >
                              {task.subject}
                            </span>
                            <span className="text-xs text-[var(--color-text-secondary)]">
                        ...
                        <button
                          onClick={() => deleteTask(dayKey, task.id)}
