...
import { Stethoscope, Plus, Trash2, Star, CheckCircle2, Circle, X, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function InternPlannerPage() {
  ...
  const totalDays = rotations.reduce((s, r) => s + diffDays(r.startDate, r.endDate), 0)
  ...
  function setRating(id: string, rating: number) {
    setRotations(prev => prev.map(r => r.id === id ? { ...r, rating } : r))
  }

  function updateNotes(id: string, notes: string) {
    setRotations(prev => prev.map(r => r.id === id ? { ...r, notes } : r))
  }
  ...
  function addSkill(rotationId: string) {
    ...
    setRotations(prev => prev.map(r => r.id === rotationId ? { ...r, skills: [...r.skills, skill] } : r))
  ...
  function toggleSkill(rotationId: string, skillId: string) {
    setRotations(prev => prev.map(r =>
      r.id === rotationId
        ? { ...r, skills: r.skills.map(s => s.id === skillId ? { ...s, done: !s.done } : s) }
        : r
    ))
  }
  ...
  function deleteSkill(rotationId: string, skillId: string) {
    setRotations(prev => prev.map(r =>
      r.id === rotationId ? { ...r, skills: r.skills.filter(s => s.id !== skillId) } : r
    ))
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-[var(--color-text-primary)]">İntern Planlayıcı</h1>
            <p className="text-[var(--color-text-secondary)] text-sm">İnternlük dönemi rotasyon takibi</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card variant="bordered" className="p-4">
          <p className="text-xs text-[var(--color-text-secondary)] mb-1">Toplam Gün</p>
          <p className="text-2xl font-bold text-[var(--color-primary)]">{totalDays}</p>
        </Card>
        <Card variant="bordered" className="p-4">
          <p className="text-xs text-[var(--color-text-secondary)] mb-1">Tamamlanan</p>
          <p className="text-2xl font-bold text-[var(--color-success)]">{completedCount}/{rotations.length}</p>
        </Card>
        <Card variant="bordered" className="p-4">
          <p className="text-xs text-[var(--color-text-secondary)] mb-1">Aktif Rotasyon</p>
          <p className="text-sm font-bold text-[var(--color-warning)] truncate">{activeRotation ? activeRotation.department : '—'}</p>
        </Card>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <Card variant="elevated" className="w-full max-w-lg mx-4 p-6">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              {[
                { label: 'Bölüm', key: 'department', placeholder: 'Dahiliye...' },
                { label: 'Hastane', key: 'hospital', placeholder: 'Hastane adı...' },
                { label: 'Sorumlu Hekim', key: 'supervisor', placeholder: 'Dr. Ad Soyad...' },
              ].map(f => (
                ...
                <div className="flex">
                  <div className="flex flex-col items-center px-4 pt-4 pb-0">
                    <div
                      ...
                      style={{ background: isActive ? 'var(--color-warning)' : rot.status === 'completed' ? 'var(--color-success)' : 'var(--color-border)' }}
                    />
                    {idx < sorted.length - 1 && <div className="w-0.5 flex-1 mt-1" style={{ background: 'var(--color-border)', minHeight: 20 }} />}
                  </div>
                  <div className="flex-1 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">{rot.hospital}{rot.supervisor ? ` · Dr. ${rot.supervisor}` : ''}</p>
                        ...
                        {rot.status === 'completed' && (
                          <div className="flex items-center gap-1 mt-1">
                            {[1, 2, 3, 4, 5].map(star => (
                              <button key={star} onClick={() => setRating(rot.id, star)}>
                                <Star
                                  size={14}
                                  fill={star <= rot.rating ? 'var(--color-warning)' : 'transparent'}
                                  style={{ color: star <= rot.rating ? 'var(--color-warning)' : 'var(--color-border)' }}
                                />
                              </button>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {rot.skills.length > 0 && (
                          <span className="text-xs text-[var(--color-text-secondary)]">{skillsDone}/{rot.skills.length} beceri</span>
                        )}
                        ...
                        <button onClick={() => deleteRotation(rot.id)}>
                          <Trash2 size={14} className="text-[var(--color-destructive)] opacity-50 hover:opacity-100" />
                        </button>
                      </div>
                    </div>
                    ...
                    {isOpen && (
                      <div className="mt-3 space-y-3 border-t border-[var(--color-border)] pt-3">
                        {/* Notes */}
                        <div>
                          <p className="text-xs font-medium text-[var(--color-text-secondary)] mb-1">Notlar</p>
                          <textarea
                            value={rot.notes}
                            onChange={e => updateNotes(rot.id, e.target.value)}
                            rows={2}
                        ...
                        {/* Skills */}
                        <div>
                          <p className="text-xs font-medium text-[var(--color-text-secondary)] mb-2">Beceri Listesi</p>
                          <div className="space-y-1.5 mb-2">
                            {rot.skills.map(sk => (
                              <div key={sk.id} className="flex items-center gap-2">
                                <button onClick={() => toggleSkill(rot.id, sk.id)}>
                                  {sk.done
                                    ? <CheckCircle2 size={14} style={{ color: 'var(--color-success)' }} />
                                    : <Circle size={14} className="text-[var(--color-text-secondary)]" />}
                                </button>
                                <span className={`text-sm flex-1 ${sk.done ? 'line-through text-[var(--color-text-secondary)]' : 'text-[var(--color-text-primary)]'}`}>{sk.label}</span>
                                <button onClick={() => deleteSkill(rot.id, sk.id)}>
                                  <X size={11} className="text-[var(--color-destructive)] opacity-40 hover:opacity-100" />
                                </button>
                              </div>
                            ))}
                          </div>
                          <div className="flex gap-2">
                            <input
                              ...
                              onChange={e => setNewSkillText(prev => ({ ...prev, [rot.id]: e.target.value }))}
                              onKeyDown={e => e.key === 'Enter' && addSkill(rot.id)}
                            ...
                            />
                            <Button size="sm" onClick={() => addSkill(rot.id)}><Plus size={12} /></Button>
                          </div>
                        </div>
                      </div>
                    )}
