...
export default function AkilliPlanlayiciPage() {
  ...
  useEffect(() => {
    const savedPlan = null /* Supabase Aktif */;
  ...
  async function generateSmartPlan() {
    ...
    try {
      const res = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({ 
          message: "Lütfen öğrenme profilimi oku. Zayıf konularıma (weakAreas) vaktimin %60'ını, güçlü konularıma %20'sini ayır. Günde en fazla 3 farklı konu belirle. Her günü Pomodoro blokları (25 dk çalışma, 5 dk mola) şeklinde planla. Pazartesi-Pazar tablosu oluştur.", 
          model: "EFFICIENT", 
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-[var(--color-warning)]/20 bg-[var(--color-warning)]/5 p-5 flex gap-4">
          ...
          <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">
            <strong>Zeka Notu:</strong> Programın %60'ı profilindeki <strong>zayıf alanlara</strong>, %20'si ise bilgini taze tutmak için <strong>güçlü alanlarına</strong> ayrılmıştır.
          </p>
        </div>
      </div>
    </div>
