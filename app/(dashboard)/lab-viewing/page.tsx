...
import {
   Sparkles, Brain, MessageSquare, Loader2, FlaskConical,
  Plus,
  Search,
  X,
  TrendingUp,
...
const REFERENCE: Record<
  string,
  { unit: string; normal: string; low: number; high: number }
> = {
  Hemoglobin: {
    unit: "g/dL",
    normal: "12-16 (K), 13.5-17.5 (E)",
    low: 12,
    high: 17.5,
  },
...
export default function LabViewingPage() {
  ...
  async function analyzeLabs() {
    ...
    setAnalyzing(true);
    const loadingId = toast.loading("Klinik bağlam ve laboratuvar verileri birleştiriliyor...");
    
    try {
      const labSummary = results.map(r => `${r.test}: ${r.value} ${r.unit} (${r.status})`).join(", ");
      
      // Harita Standardı: Hasta bağlamını sorguya ekle (Varsa mevcut vaka verisini kullanır)
      const res = await fetch("/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          message: `LABORATUVAR VERİLERİ: ${labSummary}. GÖREV: Bu sonuçları hastanın genel klinik tablosuyla ilişkilendirerek kritik bir değerlendirme yap.`, 
          model: "EFFICIENT", 
          module: "lab-viewing" 
        }),
      });
      ...
      setInterpretation(data.response.text);
      toast.success("Klinik analiz tamamlandı!", { id: loadingId });
    } catch (e) {
      toast.error("Analiz motoru şu an yanıt vermiyor.", { id: loadingId });
    } finally {
      setAnalyzing(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-[var(--color-text-primary)]">
              Lab & Görüntüleme Takip
            </h1>
          </div>
          <p className="text-[var(--color-text-secondary)]">
            Laboratuvar ve görüntüleme sonuçlarını takip edin
          </p>
        </div>
        <Button variant="secondary" onClick={analyzeLabs} disabled={analyzing || results.length === 0} className="mr-2 border-[var(--color-primary)]/30">
          {analyzing ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />}
          <span className="ml-2">AI Yorumu Al</span>
        </Button>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {["all", "critical", "high", "low", "normal"].map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeFilter === f ? "bg-[var(--color-primary)] text-black" : "bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"}`}
          >
            {f === "all"
              ? "Tümü"
              : f === "critical"
      ...
      {interpretation && (
        <Card variant="elevated" className="border-l-4 border-[var(--color-primary)] bg-[var(--color-primary)]/5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Brain size={18} className="text-[var(--color-primary)]" />
              <h3 className="font-bold text-sm uppercase tracking-wider text-[var(--color-text-primary)]">Klinik AI Değerlendirmesi</h3>
            </div>
            <button onClick={() => setInterpretation(null)} className="text-[var(--color-text-secondary)] hover:text-white">
              <X size={14} />
            </button>
          </div>
          <p className="text-sm leading-relaxed text-[var(--color-text-secondary)] whitespace-pre-wrap italic">
            {interpretation}
          </p>
          <p className="mt-3 text-[10px] text-[var(--color-text-disabled)] uppercase tracking-widest">
            * Bu analiz eğitim amaçlıdır. Klinik korelasyon hekim sorumluluğundadır.
          </p>
        
          <div className="mt-4 flex gap-2 border-t border-[var(--color-primary)]/20 pt-4">
             <Button variant="ghost" size="sm" className="text-xs hover:text-[var(--color-primary)]" onClick={() => window.location.href='/dashboard/clinic/ai-assistan'}>
               <MessageSquare size={14} className="mr-2" />
               Bulguları Klinik Asistanla Tartış
             </Button>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-[var(--color-text-secondary)] text-sm">
            Laboratuvar sonuçlarınızı ekleyerek takibe başlayın
          </p>
          ...
          {filtered.map((r) => {
            ...
            return (
              <Card
                key={r.id}
                variant="bordered"
                className="p-4 hover:border-[var(--color-primary)]/30 transition-colors"
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-2xl font-bold ${statusColor[r.status]}`}
                  >
                    {r.value}
                  </span>
