  ...
  const handleComplete = useCallback(() => {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("MEDASI Pomodoro", {
        body:
          mode === "work"
            ? "Çalışma seansı tamamlandı! Mola zamanı."
            : "Mola bitti! Çalışmaya devam et.",
        icon: "/favicon.ico",
      });
    }
  }, [mode, task, playSound]);
    ...
    <div className="flex flex-col gap-6 max-w-3xl mx-auto">
      <div className="text-center">
        ...
        <p className="text-[var(--color-text-secondary)] mt-1">
          Odaklanmış çalışma seanslarıyla verimliliğini artır
        </p>
      </div>
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="flex gap-3">
          <Button
            ...
            size="md"
            onClick={reset}
            disabled={running && timeLeft === MODES[mode].duration}
          >
            <RotateCcw size={16} />
          </Button>
          <Button
          ...
          <Button
            variant="ghost"
            size="md"
            onClick={() => changeMode(mode === "work" ? "short" : "work")}
          >
            <Coffee size={16} />
          </Button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Toplam Odaklanma",
            value: `${totalFocus} dk`,
            color: "text-[var(--color-text-primary)]",
          },
          {
            label: "Tamamlanan Mola",
            value: breaks,
          ...
          <div
            key={stat.label}
            className="text-center p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] border-t-2"
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              Henüz seans tamamlanmadı
            </p>
            <p className="text-[var(--color-text-secondary)] text-xs mt-1">
              İlk Pomodoronuzu başlatın!
            </p>
          </div>
        ) : (
