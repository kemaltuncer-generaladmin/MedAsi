...
import {
  XCircle, CheckCircle, Play, Plus, X, ChevronRight,
  BookOpen, Calendar, Filter, Trash2
} from 'lucide-react'
...
function difficultyBadgeVariant(d: string): 'success' | 'warning' | 'destructive' {
...
export default function HataliSorularPage() {
  ...
  const allSubjects = Array.from(new Set(activeWrongs.map(w => w.questionSnapshot.subject)))

  const displayWrongs = (showLearned ? learnedWrongs : activeWrongs).filter(w =>
  ...
  const learnedThisWeek = learnedWrongs.filter(w => new Date(w.addedAt) >= oneWeekAgo).length
  ...
  function startPractice() {
    const queue = activeWrongs.filter(w =>
      filterSubject === 'Tümü' || w.questionSnapshot.subject === filterSubject
    )
    if (queue.length === 0) {
  ...
  function nextQuestion() {
    if (practiceIndex >= practiceQueue.length - 1) {
      setMode('list')
      toast.success(`Tekrar tamamlandı! Doğru: ${practiceStats.correct}, Yanlış: ${practiceStats.wrong}`)
      return
    }
  ...
  function addManual() {
    if (!manualForm.text.trim() || !manualForm.optionA || !manualForm.optionB || !manualForm.optionC || !manualForm.optionD) {
    ...
    setManualForm({ text: '', optionA: '', optionB: '', optionC: '', optionD: '', correct: '0', userAnswer: '1', subject: 'Diğer', difficulty: 'Orta', explanation: '' })
    setMode('list')
      ...
      <div className="flex flex-col gap-5 max-w-2xl mx-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Tekrar Çalış</h1>
        ...
        <div className="flex gap-3">
          <div className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-3 text-center">
            <p className="text-xl font-bold text-[var(--color-success)]">{practiceStats.correct}</p>
            <p className="text-xs text-[var(--color-text-secondary)]">Doğru</p>
          </div>
          <div className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-3 text-center">
            <p className="text-xl font-bold text-[var(--color-destructive)]">{practiceStats.wrong}</p>
        ...
        <Card variant="elevated" className="p-6">
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <Badge variant="destructive" className="ml-auto">Hatalı Soru</Badge>
          </div>
          <p className="text-[var(--color-text-primary)] text-lg font-medium leading-relaxed mb-6">{q.text}</p>

          <div className="space-y-3">
            {q.options.map((opt, idx) => {
              let cls = 'w-full text-left px-4 py-3 rounded-lg border text-sm font-medium transition-all '
              if (selectedOption === null) {
                cls += idx === current.userAnswer
              ...
              return (
                <button key={idx} className={cls} onClick={() => handleSelectOption(idx)}>
                  ...
                  {selectedOption !== null && idx === q.correct && <CheckCircle size={16} className="inline ml-2" />}
                  {selectedOption === idx && idx !== q.correct && <XCircle size={16} className="inline ml-2" />}
                </button>
              )
            })}
          </div>
          ...
          {showExplanation && q.explanation && (
            <div className="mt-5 p-4 rounded-lg bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20">
              ...
              <p className="text-sm text-[var(--color-text-primary)] leading-relaxed">{q.explanation}</p>
            </div>
          )}
        </Card>
        ...
        {selectedOption !== null && (
          <div className="flex gap-3">
            <Button
              variant="secondary"
              size="sm"
              onClick={() => { markLearned(current.id); nextQuestion() }}
              className="flex-1"
            >
      ...
      <div className="flex flex-col gap-5 max-w-2xl mx-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Manuel Ekle</h1>
              <p className="text-[var(--color-text-secondary)] text-sm">Hatalı soru manuel olarak ekle</p>
            </div>
          </div>
          <Button variant="ghost" size="sm" onClick={() => setMode('list')}>
            <X size={15} /> İptal
        ...
        <Card variant="elevated" className="p-6 space-y-4">
          ...
          {(['A', 'B', 'C', 'D'] as const).map((letter, idx) => {
            ...
            return (
              <div key={letter}>
                <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-1.5">Seçenek {letter}</label>
                <input
                  type="text"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setManualForm(f => ({ ...f, difficulty: e.target.value as Difficulty }))}
                className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] transition-colors"
              >
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-[var(--color-text-primary)]">Hatalı Sorular</h1>
            <p className="text-[var(--color-text-secondary)] text-sm">{activeWrongs.length} tekrar edilecek soru</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card variant="bordered" className="p-4 text-center">
          <XCircle size={16} className="text-[var(--color-destructive)] mx-auto mb-1" />
          <p className="text-2xl font-bold text-[var(--color-text-primary)]">{activeWrongs.length}</p>
          <p className="text-xs text-[var(--color-text-secondary)]">Toplam Hatalı</p>
        </Card>
        <Card variant="bordered" className="p-4 text-center">
          <CheckCircle size={16} className="text-[var(--color-success)] mx-auto mb-1" />
          <p className="text-2xl font-bold text-[var(--color-text-primary)]">{learnedWrongs.length}</p>
          <p className="text-xs text-[var(--color-text-secondary)]">Öğrenildi</p>
        </Card>
        <Card variant="bordered" className="p-4 text-center">
          <Calendar size={16} className="text-[var(--color-primary)] mx-auto mb-1" />
          <p className="text-2xl font-bold text-[var(--color-text-primary)]">{learnedThisWeek}</p>
          <p className="text-xs text-[var(--color-text-secondary)]">Bu Hafta Öğrenildi</p>
        </Card>
        <Card variant="bordered" className="p-4 text-center">
          <BookOpen size={16} className="text-[var(--color-warning)] mx-auto mb-1" />
          <p className="text-2xl font-bold text-[var(--color-text-primary)]">{allSubjects.length}</p>
          <p className="text-xs text-[var(--color-text-secondary)]">Konu Sayısı</p>
        </Card>
      </div>
      ...
      {displayWrongs.length === 0 ? (
        <Card variant="bordered" className="p-8 text-center">
          <CheckCircle size={32} className="text-[var(--color-success)] mx-auto mb-3 opacity-60" />
          <p className="text-[var(--color-text-primary)] font-medium">
            {showLearned ? 'Henüz öğrenilmiş soru yok.' : 'Harika! Hatalı sorunuz bulunmuyor.'}
          </p>
          <p className="text-sm text-[var(--color-text-secondary)] mt-1">
            {!showLearned && 'Soru bankasında pratik yaparak hatalı soruları takip edin.'}
          </p>
        </Card>
      ) : (
        Object.entries(grouped).map(([subject, items]) => (
          <div key={subject}>
            <div className="flex items-center gap-2 mb-2">
              <h2 className="text-sm font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide">{subject}</h2>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          ...
                          <span className="text-xs text-[var(--color-text-secondary)]">
                            {new Date(wrong.addedAt).toLocaleDateString('tr-TR')}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                          {q.options.map((opt, idx) => {
                            ...
                            if (idx === q.correct) {
                              cls += 'border-[var(--color-success)]/40 bg-[var(--color-success)]/5 text-[var(--color-success)]'
                            } else if (idx === wrong.userAnswer && idx !== q.correct) {
                              cls += 'border-[var(--color-destructive)]/40 bg-[var(--color-destructive)]/5 text-[var(--color-destructive)]'
                            } else {
                              cls += 'border-[var(--color-border)] text-[var(--color-text-secondary)]'
                            }
                            return (
                              <div key={idx} className={cls}>
                                <span className="font-bold">{OPTION_LABELS[idx]}.</span> {opt}
                                {idx === q.correct && <span className="ml-1 text-[0.65rem]">(Doğru)</span>}
                                {idx === wrong.userAnswer && idx !== q.correct && <span className="ml-1 text-[0.65rem]">(Seçtiğin)</span>}
                              </div>
                            )
                          })}
                        </div>
                        {q.explanation && (
                          <div className="mt-2 p-2 rounded bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20">
                            <p className="text-xs text-[var(--color-text-secondary)]">{q.explanation}</p>
                          </div>
                        )}
                      </div>
