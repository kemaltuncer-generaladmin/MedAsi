...
type Category = TusSubjectOrOther;

const CATEGORIES: Category[] = [...TUS_SUBJECTS_WITH_OTHER];

const CATEGORY_COLORS: Record<Category, string> = TUS_SUBJECT_BADGES;
...
export default function DersNotlariPage() {
  ...
  useEffect(() => {
    if (showAiPanel && materials.length === 0) {
      fetchMaterials();
    }
  }, [showAiPanel, materials.length]);
  ...
  async function handleAiSummary() {
    ...
    if (!mat) {
      toast.error("Lütfen bir materyal seçin");
      return;
    }
    setAiLoading(true);
    setAiResult("");
    const loadingId = toast.loading("AI özet oluşturuyor...");
    try {
      const res = await fetch("/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: `Şu materyalin içeriğinden ders notu formatında özet çıkar: "${mat.name}". Başlıklar, madde işaretleri ve önemli noktaları vurgula.`,
          model: "EFFICIENT",
          module: "ders-notlari",
          materialIds: [mat.id],
        }),
      ...
      setAiResult(text);
      toast.success("Özet oluşturuldu!", { id: loadingId });
    } catch {
      toast.error("AI özet oluşturulamadı", { id: loadingId });
    } finally {
      setAiLoading(false);
    }
  }
  ...
  function saveAiResultAsNote() {
    ...
    const newNote: Note = {
      ...
      title: mat ? `AI Özet: ${mat.name}` : `AI Özet (${now})`,
      content: aiResult,
      category: "Diğer",
      tags: ["ai-özet"],
      date: now,
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)]">
            Kişisel ders notlarınızı oluşturun ve yönetin
          </p>
        </div>
        ...
        <Card variant="bordered" className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button
              ...
              disabled={aiLoading || !selectedMaterialId}
            >
              {aiLoading ? (
                <Loader2 size={14} className="animate-spin" />
              ) : (
          ...
          {aiResult && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <Button variant="secondary" size="sm" onClick={copyAiResult}>
                  <Copy size={13} />
                  Kopyala
                </Button>
                <Button variant="primary" size="sm" onClick={saveAiResultAsNote}>
                  <Download size={13} />
                  Notu Kaydet
                </Button>
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Etiket",
            value: new Set(notes.flatMap((n) => n.tags)).size,
          },
        ].map((s) => (
            ...
            >
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <p className="font-semibold text-[var(--color-text-primary)] leading-tight line-clamp-2 flex-1">
                    {note.title}
                  </p>
                  <Badge
                    variant={
                      CATEGORY_COLORS[note.category] as
                        | "default"
                        | "success"
                        | "warning"
                        | "destructive"
                ...
                {note.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {note.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] text-xs"
                      >
                        <Tag size={9} />
                        {tag}
                      </span>
                ...
                <div className="flex items-center justify-between pt-1 border-t border-[var(--color-border)]">
                  ...
                  >
                    <button
                      ...
                      className="p-1.5 rounded hover:bg-[var(--color-surface-elevated)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
                    >
                      <Download size={13} />
                    </button>
                    <button
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--color-surface-elevated)] rounded-xl w-full max-w-2xl border border-[var(--color-border)] flex flex-col max-h-[90vh]">
            <div className="flex items-start justify-between p-6 border-b border-[var(--color-border)]">
              <div className="flex-1 pr-4">
                <div className="flex items-center gap-2 mb-1">
                  <Badge
                    variant={
                      CATEGORY_COLORS[viewNote.category] as
                        | "default"
                        | "success"
                        | "warning"
                        | "destructive"
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button
                  ...
                  className="p-2 rounded hover:bg-[var(--color-surface)] text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors"
                >
                  <Edit2 size={16} />
                </button>
                <button
                  onClick={() => handleDelete(viewNote.id)}
                  className="p-2 rounded hover:bg-[var(--color-surface)] text-[var(--color-text-secondary)] hover:text-[var(--color-destructive)] transition-colors"
                >
                  <Trash2 size={16} />
                </button>
                <button
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <p className="text-[var(--color-text-primary)] whitespace-pre-wrap leading-relaxed text-sm">
                {viewNote.content}
              </p>
              ...
              {form.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {form.tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] text-xs cursor-pointer"
