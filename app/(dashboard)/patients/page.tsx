...
type Patient = { id: string; name: string; noteField?: string | null; createdAt: string }
...
export default function PatientsPage() {
  ...
  async function save() {
    if (!form.name.trim()) { toast.error('Hasta adı zorunludur'); return }
    setSaving(true)
    try {
      const res = await fetch('/api/patients', {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-[var(--color-text-primary)]">Hastalarım</h1>
          </div>
          <p className="text-[var(--color-text-secondary)]">Tüm kayıtlı hastalarınız</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge variant="default" className="shrink-0">{filtered.length} hasta</Badge>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-40 rounded-lg bg-[var(--color-surface)] animate-pulse border border-[var(--color-border)]" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-[var(--color-text-secondary)] text-sm mt-1">
              {search ? `"${search}" için hasta bulunamadı` : 'İlk hastanızı ekleyin'}
            </p>
          </div>
          {!search && (
            <Button variant="primary" onClick={openCreate}>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-[var(--color-text-primary)] truncate">{patient.name}</p>
                  <div className="flex items-center gap-1.5 mt-1">
                    <Calendar size={11} className="text-[var(--color-text-secondary)]" />
                    <p className="text-xs text-[var(--color-text-secondary)]">{formatDate(patient.createdAt)}</p>
                  </div>
                </div>
              </div>
              {patient.noteField && (
              ...
              <div className="flex gap-2 mt-4 pt-3 border-t border-[var(--color-border)]">
                <Button variant="ghost" size="sm" onClick={() => openEdit(patient)} className="flex-1">
                  <Edit2 size={13} />
                  Düzenle
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setConfirmDelete(patient)} className="flex-1 hover:text-[var(--color-destructive)]">
                  <Trash2 size={13} />
                  Sil
                </Button>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => setForm(f => ({ ...f, noteField: e.target.value }))}
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--color-surface-elevated)] rounded-xl p-6 w-full max-w-sm border border-[var(--color-border)]">
            <h2 className="text-lg font-bold text-[var(--color-text-primary)] mb-2">Hastayı Sil</h2>
            <p className="text-[var(--color-text-secondary)] text-sm mb-6">
              <span className="text-[var(--color-text-primary)] font-medium">{confirmDelete.name}</span> adlı hastayı silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.
            </p>
