    ...
    <span className="flex gap-0.5">
      {[1, 2, 3].map((i) => (
        <Star
          ...
          stroke={
            i <= count ? "var(--color-warning)" : "var(--color-text-secondary)"
          }
          className={
            interactive
          ...
          onClick={
            interactive && onChange ? () => onChange(i as 1 | 2 | 3) : undefined
          }
        />
      ))}
    </span>
...
export default function SpotNotlarPage() {
  ...
  const filtered = notes
    .filter((n) => {
      ...
      if (filterImportance > 0 && n.importance !== filterImportance)
        return false;
      if (
        search &&
        !n.title.toLowerCase().includes(search.toLowerCase()) &&
        !n.fact.toLowerCase().includes(search.toLowerCase())
      )
        return false;
      return true;
    })
          ...
          {!revealed ? (
            <div className="text-center py-8">
              <p className="text-[var(--color-text-secondary)] text-sm mb-4">
                Hatırlamaya çalış, sonra yanıtı göster
              </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)] focus:outline-none focus:border-[var(--color-primary)] transition-colors"
              >
                {TUS_SUBJECTS_WITH_OTHER.map((subject) => (
            ...
            <div>
              ...
              <Stars
                count={form.importance}
                interactive
                onChange={(n) => setForm((f) => ({ ...f, importance: n }))}
              />
            </div>
          </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              {activeCount} aktif · {memorizedCount} ezberlenmiş
            </p>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <Card variant="bordered" className="p-8 text-center">
          ...
          <p className="text-[var(--color-text-secondary)]">
            {showMemorized
              ? "Henüz ezberlenmiş spot not yok."
              : "Kriterlere uygun spot not bulunamadı."}
          </p>
        </Card>
      ) : (
            ...
            <Card key={note.id} variant="bordered" className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {note.memorized && (
                    <Badge variant="success">Ezberlenmiş</Badge>
                  )}
                </div>
