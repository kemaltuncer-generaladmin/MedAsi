...
const SAMPLE_DECK: Deck = {
  id: "sample_dahiliye",
  name: "Dahiliye Kardiyoloji Terimleri",
  subject: "Dahiliye",
  color: "#ef4444",
...
export default function FlashcardPage() {
  function getNextReviewDate(rating: "unknown" | "hard" | "known") {
    const now = new Date();
    if (rating === "known") now.setDate(now.getDate() + 4); // İyi bildiğini 4 gün sonra sor
    else if (rating === "hard") now.setDate(now.getDate() + 1); // Zorlandığını yarın sor
    else now.setMinutes(now.getMinutes() + 30); // Bilmediğini 30 dk sonra sor
  ...
  async function generateFromNotes() {
    const loadingId = toast.loading("Ders notların analiz ediliyor...");
    try {
      const res = await fetch("/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          message: "Son aldığım klinik notları özetle ve onlardan 5 adet spot flashcard üret.", 
          model: "EFFICIENT", 
          module: "source-ai-notlar" 
        }),
      });
      const data = await res.json();
      toast.success("Notların flashcard'a dönüştürüldü!", { id: loadingId });
      // Mevcut generateAICards fonksiyonunu tetikle veya veriyi işle
    } catch (e) {
      toast.error("Notlar bulunamadı veya işlenemedi.");
    }
  }

  async function generateAICards() {
    const loadingId = toast.loading("Merkezi beyin profilini analiz ediyor...");
    try {
      const res = await fetch("/api/ai/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          message: "Lütfen öğrenme profilimi analiz et ve en zayıf olduğum konulardan 5 adet kritik flashcard üret. Format: KART 1 - Ön: [Soru] - Arka: [Cevap]. Sadece kartları dön.", 
          model: "EFFICIENT", 
          module: "flashcards-ai" 
        }),
      });

      if (!res.ok) throw new Error("Beyin şu an yanıt vermiyor.");

      const data = await res.json();
      const aiText = data.response.text;

      // Harita Standardı: Kartları parçala ve yapılandır
      const cardBlocks = aiText.split(/KART\s*\d+/i).filter((b: string) => b.trim().length > 5);
      const newCards = cardBlocks.map((block: string, idx: number) => {
        const lines = block.split('\n').filter((l: string) => l.trim());
        const front = lines.find((l: string) => l.toLowerCase().includes('ön:'))?.replace(/ön:/i, '').trim() || "Kavram bulunamadı";
        const back = lines.find((l: string) => l.toLowerCase().includes('arka:'))?.replace(/arka:/i, '').trim() || "Detay bulunamadı";
        return { 
      ...
      if (newCards.length === 0) throw new Error("Format hatası oluştu.");

      // Harita Standardı: Kişiselleştirilmiş Deste Yönetimi
      const aiDeckId = "ai_suggestions_deck";
      let updatedDecks;
      const existingAiDeck = decks.find(d => d.id === aiDeckId);

      if (existingAiDeck) {
        updatedDecks = decks.map(d => d.id === aiDeckId ? { 
          ...d, 
          cards: [...newCards, ...d.cards].slice(0, 50),
          lastStudied: new Date().toISOString()
        } : d);
      } else {
        const newDeck = {
          id: aiDeckId,
          name: "AI Önerileri",
          subject: "Kişiselleştirilmiş",
          color: "#8b5cf6", // Haritadaki mor vurgu
          cards: newCards,
          createdAt: new Date().toISOString(),
          lastStudied: new Date().toISOString()
        };
      ...
      toast.success("Profiline göre 5 yeni kart eklendi!", { id: loadingId });
    } catch (e) {
      console.error(e);
      toast.error("AI kart üretemedi, lütfen tekrar deneyin.", { id: loadingId });
    }
  }

  const [decks, setDecks] = useState<Deck[]>([]);
  const [mode, setMode] = useState<"list" | "deck" | "study" | "addDeck">(
    "list",
  );
  ...
  function rateCard(rating: "unknown" | "hard" | "known") {
      ...
      return {
        ...
        cards: d.cards.map((c) =>
          c.id === studyQueue[studyIndex].id
            ? { ...c, rating, lastStudied: new Date().toISOString(), nextReview: getNextReviewDate(rating) }
            : c,
        ),
      };
    });
    ...
    if (studyIndex >= studyQueue.length - 1) {
      const total =
        sessionStats.known + sessionStats.hard + sessionStats.unknown + 1;
      ...
      <div className="flex flex-col gap-5 max-w-2xl mx-auto">
        ...
        <div className="flex gap-3">
          {[
            ...
            {
              label: "Bilmedim",
              count: sessionStats.unknown,
              color: "var(--color-destructive)",
            },
          ].map((s) => (
            <div
              key={s.label}
              className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-3 text-center"
            >
          ...
          >
            ...
            >
              <p className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wide mb-4">
                Ön Yüz — Tıkla ve çevir
              </p>
              <p className="text-xl font-bold text-[var(--color-text-primary)] leading-relaxed whitespace-pre-line">
                {currentCard.front}
              </p>
              <div
        ...
        <Card variant="elevated" className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setDeckForm((f) => ({ ...f, subject: e.target.value }))
              }
              ...
              {DECK_COLORS.map((color) => (
                <button
                  ...
                  style={{
                    background: color,
                    borderColor:
                      deckForm.color === color ? "white" : "transparent",
                    transform:
                      deckForm.color === color ? "scale(1.15)" : "scale(1)",
                  }}
                />
              ))}
            </div>
  ...
  if (mode === "deck" && selectedDeck) {
    ...
    const unknownCount = deck.cards.filter(
      (c) => c.rating === "unknown",
    ).length;
      ...
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[var(--color-text-secondary)] text-sm">
                {deck.cards.length} kart · {deck.subject}
              </p>
            </div>
          </div>
        ...
        {deck.cards.length > 0 && (
          <div className="flex gap-3">
            ...
            <div className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-3 text-center">
              <p className="text-lg font-bold text-[var(--color-destructive)]">
                {unknownCount}
              </p>
              <p className="text-xs text-[var(--color-text-secondary)]">
                Bilmedim
          ...
          {deck.cards.map((card) => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--color-text-secondary)] mt-1 whitespace-pre-line">
                  {card.back}
                </p>
                {card.rating && (
                  <span
        ...
        <button
          onClick={() => {
            if (confirm("Bu desteyi silmek istediğinizden emin misiniz?"))
              deleteDeck(deck.id);
          }}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] text-sm">
              {decks.length} deste ·{" "}
              {decks.reduce((s, d) => s + d.cards.length, 0)} kart
            </p>
          </div>
        </div>
        <Button variant="secondary" size="sm" onClick={generateAICards} className="mr-2">
          <Sparkles size={14} className="mr-2" /> AI Önerisi
        </Button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {decks.map((deck) => {
            const knownPct =
              deck.cards.length > 0
                ? Math.round(
                    (deck.cards.filter((c) => c.rating === "known").length /
                      deck.cards.length) *
                      100,
                  )
                : 0;
