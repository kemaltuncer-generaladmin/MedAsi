...
let lineCounter = 0;
const newLine = (type: LineType, text: string): TerminalLine => ({
  type,
  text,
  id: lineCounter++,
});
  ...
  async function processCommand(cmd: string) {
    ...
    if (aiCommands[command]) {
      if (!argStr && command !== "vitals") {
        addLine(
          "error",
          `Hata: '${command}' komutu için argüman gerekli. Örn: ${command} [argüman]`,
        );
        return;
      }
      setLoading(true);
      ...
      try {
        ...
        setLines((prev) => {
          ...
          return [
            ...withoutProcessing,
            newLine("error", err instanceof Error ? err.message : "AI hatası"),
          ];
        });
      } finally {
        setLoading(false);
    ...
    addLine(
      "error",
      `'${command}': komut bulunamadı. 'help' yazarak mevcut komutları görün.`,
    );
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-xs text-[var(--color-text-secondary)]">
          <span className="px-2 py-1 rounded bg-[var(--color-surface)] border border-[var(--color-border)] font-mono">
            EFFICIENT Model
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 font-mono text-sm space-y-0.5">
          {lines.map((line) => (
            <div
              key={line.id}
              className={`leading-relaxed whitespace-pre-wrap ${lineColor[line.type]}`}
            >
              {line.text}
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t border-[var(--color-border)]/50">
          <span className="font-mono text-sm text-[var(--color-primary)] shrink-0">
            medasi@terminal:~$
          </span>
          <input
            ref={inputRef}
      ...
      <div className="flex flex-wrap gap-2">
        {[
          "diagnose ateş baş ağrısı",
          "drugs aspirin",
          "labs troponin",
          "icd miyokard enfarktüsü",
          "vitals 35 female",
        ].map((cmd) => (
          <button
            key={cmd}
