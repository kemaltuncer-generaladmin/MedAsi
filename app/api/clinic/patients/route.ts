...
export async function PUT(req: Request) {
  ...
  if (index === -1) return NextResponse.json({ error: 'Hasta bulunamadı' }, { status: 404 })
  globalAny[key][index] = { ...globalAny[key][index], ...parsed.data, updatedAt: new Date().toISOString() }
...
export async function DELETE(req: Request) {
  ...
  if (before === globalAny[key].length) return NextResponse.json({ error: 'Hasta bulunamadı' }, { status: 404 })
